...
function schedule(state, grade, now = Date.now()) {
...
function due(cards, states, now = Date.now(), kind = null) {
  return cards
...
const key = card => `${card.topicID}:${card.front}`;
...
const load = async path => {
  if (!cache.has(path)) {
    cache.set(path, fetch(`data/${path}?v=${VERSION}`).then(r => r.json()));
  }
...
async function renderTopics() {
  ...
  const foot = el('div', 'card flat');
  foot.append(el('div', 'muted small', `версія ${VERSION}`));
  view.append(foot);
}
